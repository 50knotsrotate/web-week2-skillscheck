...
function bigOrSmall(arr) {
	...
	for (let number of arr) {
		number > 100 ? answers.push('big') : answers.push('small')
	}
	return answers
}
...
var doubled = myNumbers.map(number => number * 2)
...
var filtered = myNumbers.filter(number => number > 100 )
...
var total = myNumbers.reduce((bathman, superbman) => bathman + superbman)
...
myNumbers.forEach((Eric_Cartman, the_beach_bots) => myNumbersIndex.push(the_beach_bots))
...
var forTheLoveOfGeorge = notGeorge.map(who_is_that_guy_on_the_one_dollar_bill_again_I_cant_seem_to_remember => 'George')
...
var totallyAwesome = people.reduce((acc, curr) => curr.awesomeLevel + acc, 0)
